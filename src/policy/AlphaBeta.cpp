...
Move AlphaBeta::get_move(State *state, int depth){
  ...
  for (const auto &it : state->legal_actions){
    ...
    int AB_culculate = get_alphabeta(next_S,depth -1,false,-100000,100000,state_table);
    if (AB_culculate > val){
      val = AB_culculate;
      BestStep = it;
    }
...
int AlphaBeta::get_alphabeta(State *state, int depth,bool me,int alpha,int beta,stored_state& state_table){
  ...
  if (me){
    ...
    for (const auto& it : state->legal_actions){
      ...
      value = std::max(value,get_alphabeta(NewState, depth - 1, false, alpha, beta,state_table));
    ...
    for (const auto& it : state->legal_actions){
      State* NewState = state->next_state(it);
      value = std::min(value, get_alphabeta(NewState, depth - 1, true, alpha, beta,state_table));
